	...
	public class GameController : Singleton<GameController>
	{
		...
		internal void MachineMode(bool machineMode)
		{
			...
			if (machineMode)
			{
				...
				// Activamos el "Machine Controller":
				_machine.enabled = true;
                // Activamos el "Weapon Controller" (lo activamos desde otro script. Además, es la forma correcta debido a que el "Machine Controller" tiene referencias a este):
			...
			{
				// Si no estamos conduciendo la Máquina, :
		...
		// Método que se invocará cuando estés en el Modo Máquina:
        private void DontShowAvatar()
        {
            PhotonView photonView = _player.GetComponent<PhotonView>();
			photonView.RPC("ShowAvatar", RpcTarget.Others, false);
        }

        // Método que se invocará cuando no estés en el Modo Máquina:
        private void ShowAvatar()
        {
            PhotonView photonView = _player.GetComponent<PhotonView>();
			photonView.RPC("ShowAvatar", RpcTarget.Others, true);
        }
        #endregion
        ...
        private void SeeAvatar(bool show)
        {
